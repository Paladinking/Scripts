        ...
        for (unsigned i = 0; i < read; ++i) {
            if (read_buffer[i] == '{') {
                ...
                if (key_buffer.length == 0) {
                    ...
                    MapEntry *entry =
                        binsearch(map, map_len, key_buffer.buffer);
                    if (entry == NULL) {
                        WriteFile(out, "{{", 2, NULL, NULL);
                        WriteFile(out, key_buffer.buffer, key_buffer.length,
                                  NULL, NULL);
                        WriteFile(out, "}}", 2, NULL, NULL);
                    } else {
                        WriteFile(out, entry->value, entry->value_len, NULL,
                                  NULL);
                    }
                }
                ...
                if (closed_template == 1) {
                    WriteFile(out, "{{", 2, NULL, NULL);
                    WriteFile(out, key_buffer.buffer, key_buffer.length, NULL,
                              NULL);
...
int main() {
    ...
    template_file = CreateFile(argv[1], GENERIC_READ, FILE_SHARE_READ, NULL,
    ...
    HANDLE translation_file = CreateFile(argv[2], GENERIC_READ, FILE_SHARE_READ,
    ...
    if (s == PARSE_SUCCES) {
        HANDLE out_file =
            CreateFile(argv[3], GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, 0, NULL);
