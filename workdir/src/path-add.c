...
OpStatus add_paths(LPSTR paths, DWORD count, LPSTR name) {
	...
	for (unsigned i = 0; i < count; ++i) {
		...
		if ((line_name = strchr(line, '|')) != NULL && _stricmp(line_name + 1, name) == 0) {
			...
			LPWSTR path = NarrowStringToWide(line, &len);
			OpStatus status = path_add(path, &path_buffer, TRUE, FALSE);
...
int main() {
	...
	for (DWORD i = 1; i < argc; ++i) {
		...
		LPSTR narrow = WideStringToNarrow(argv[i], &len);
		if (add_paths(paths_data, lines, narrow) == OP_SUCCESS) {
			changed = TRUE;
		}
		HeapFree(heap, 0, narrow);
	}
